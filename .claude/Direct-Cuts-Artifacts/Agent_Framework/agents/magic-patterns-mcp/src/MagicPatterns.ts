          ...
          create_design: Effect.fn(function* ({ prompt, mode, presetId }) {
            ...
            const response = yield* pipe(
              ...
              Effect.catchAll((error) => {
                // Handle errors gracefully instead of crashing the server
                const errorMessage =
                  error._tag === "ParseError"
                    ? "Invalid response format from Magic Patterns API. The API response did not match the expected schema."
                    : error instanceof Error
                      ? error.message
                      : String(error);

                return Effect.fail(
                  new Error(`Failed to create design via Magic Patterns API: ${errorMessage}`),
                );
              }),
            )
