  ...
  return (
    <section className="bg-slate-50 px-4 py-16 md:py-20">
      <div className="mx-auto max-w-6xl">
        <h2 className="text-2xl font-bold text-navy-900 md:text-3xl">Modules</h2>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {QSUITE_MODULES.map((m) => (
            <article key={m.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">{m.name}</p>
              <h3 className="mt-2 text-lg font-semibold text-navy-900">{m.fullName}</h3>
              <p className="mt-2 text-sm text-slate-600">{m.description}</p>
              <p className="mt-4 text-sm font-medium text-navy-800">{outcomes[m.id]}</p>
            </article>
          ))}
        </div>
      </div>
