...
// Validation schema for Lead Leak Check intake
const leadLeakCheckSchema = z.object({
  ...
  whatsBreaking: z.string().max(2000),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Log error but don't block legitimate form submissions
    ...
    // Validate input
    const validated = leadLeakCheckSchema.parse(body);
    ...
    const sanitized = {
      ...
      whatsBreaking: sanitizeText(validated.whatsBreaking, 2000),
    };
    ...
    console.error('[Lead Leak Check] Intake error:', error);
    return NextResponse.json(
