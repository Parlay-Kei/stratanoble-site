...
3. **Link to project**:
   ```bash
   # IMPORTANT: The canonical migrations live under `supabase/migrations` at the
   # repo root (see docs/technical/migration/migration-guide.md).
   #
   # We link from the repo root so Supabase CLI picks up `supabase/config.toml`
...
curl https://REDACTED.supabase.co/rest/v1/

# Check auth status
curl https://REDACTED.supabase.co/auth/v1/health
```
